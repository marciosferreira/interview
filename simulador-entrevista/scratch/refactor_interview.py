...
content = re.sub(r'  <!-- Fonts & Icons -->.*?</header>\s*<!-- Mobile drawer -->\s*<div id="mobile-drawer">.*?</div>', new_head, content, flags=re.DOTALL)
...
# Replace icons in bubble buttons
...
# Replace icons in scorecard html
...
# Update inputs and buttons html
content = content.replace('<button id="pause-btn" title="Pause / Resume audio" data-i18n="interview.tts.pause">⏸ Pause</button>', '<button id="pause-btn" title="Pause / Resume audio"><i data-lucide="pause" style="width:14px;height:14px"></i> <span data-i18n="interview.tts.pause">Pause</span></button>')
content = content.replace('<button id="skip-btn" title="Skip audio" data-i18n="interview.tts.skip">⏭ Skip</button>', '<button id="skip-btn" title="Skip audio"><i data-lucide="skip-forward" style="width:14px;height:14px"></i> <span data-i18n="interview.tts.skip">Skip</span></button>')

content = content.replace('<button id="mic-btn" class="icon-btn" disabled title="Click to start/stop recording">🎙️</button>', '<button id="mic-btn" class="icon-btn" disabled title="Click to start/stop recording"><i data-lucide="mic"></i></button>')
content = content.replace('<button id="cancel-rec-btn" class="icon-btn" title="Cancel recording">✕</button>', '<button id="cancel-rec-btn" class="icon-btn" title="Cancel recording"><i data-lucide="x"></i></button>')
content = content.replace('<button id="send-btn" class="icon-btn" disabled title="Send (Enter)">➤</button>', '<button id="send-btn" class="icon-btn" disabled title="Send (Enter)"><i data-lucide="send"></i></button>')

content = content.replace('<button id="ap-play" title="Play / Pause">▶</button>', '<button id="ap-play" title="Play / Pause"><i data-lucide="play" style="width:20px;height:20px"></i></button>')
content = content.replace('<button id="ap-discard" data-i18n="interview.audio.discard">✕ Discard</button>', '<button id="ap-discard"><i data-lucide="x" style="width:14px;height:14px"></i> <span data-i18n="interview.audio.discard">Discard</span></button>')
content = content.replace('<button id="ap-send" data-i18n="interview.audio.send">➤ Send</button>', '<button id="ap-send"><i data-lucide="send" style="width:14px;height:14px"></i> <span data-i18n="interview.audio.send">Send</span></button>')

# Call lucide.createIcons() at the end
content = content.replace("</script>\n<script src=\"/mobile-menu.js\">", "  lucide.createIcons();\n</script>\n<script src=\"/mobile-menu.js\">")

# Add spinner to scorecard loading
...
# Also fix the scorecard error message styling
...
content = content.replace("<h2 style=\"font-size:15px;color:#f1f5f9;margin-bottom:8px\">", "<h2 style=\"font-size:18px;color:var(--text-primary);margin-bottom:12px\">")
content = content.replace("<p style=\"color:#64748b;font-size:13px;max-width:380px;margin:0 auto 20px\">", "<p style=\"color:var(--text-secondary);font-size:15px;max-width:460px;margin:0 auto 24px;line-height:1.6\">")
content = content.replace("<a href=\"/upgrade.html\" style=\"display:inline-block;padding:10px 24px;border-radius:20px;background:linear-gradient(135deg,#6366f1,#7c3aed);color:#fff;font-size:13px;font-weight:600;text-decoration:none\">", "<a href=\"/upgrade.html\" class=\"btn btn-primary\" style=\"padding:12px 24px;font-size:15px\">")
